...
// 	class Config {
// 	  constructor() {
// 	    this.apiKey = process.env.PAYGLOCAL_API_KEY || this.throwError('Missing apiKey');
// 	    this.merchantId = process.env.PAYGLOCAL_MERCHANT_ID || this.throwError('Missing merchantId');
...
// 	    this.baseUrl = process.env.PAYGLOCAL_BASE_URL || 'https://api.uat.payglocal.in';
// 	    this.logLevel = process.env.PAYGLOCAL_LOG_LEVEL || 'info';
// 	  }
// 	  throwError(message) {
// 	    throw new Error(`Missing required config field: ${message}`);
// 	  }
// 	}
// 	config = Config;
// 	return config;
...
// var webapi = /*#__PURE__*/Object.freeze({
// 	__proto__: null,
...
// var require$$0$1 = /*@__PURE__*/getAugmentedNamespace(webapi);

// var crypto_1;
// var hasRequiredCrypto;

// function requireCrypto () {
// 	if (hasRequiredCrypto) return crypto_1;
// 	hasRequiredCrypto = 1;
// 	const jose = require$$0$1;
...
// 	    : await jose.importSPKI(pem, 'RSA-OAEP-256');
// 	}
...
// 	  return await new jose.CompactSign(new TextEncoder().encode(JSON.stringify(digestObject)))
// 	    .setProtectedHeader({
// 	      'issued-by': config.merchantId,
// 	      alg: 'RS256',
...
// 	crypto_1 = { pemToKey, generateJWE, generateJWS };
// 	return crypto_1;
// }
...
// 	  value: true
// 	});
// 	cjs.isNetworkError = isNetworkError;
...
// 	cjs.default = axiosRetry;
// 	cjs.DEFAULT_OPTIONS = cjs.namespace = void 0;
...
// 	cjs.DEFAULT_OPTIONS = DEFAULT_OPTIONS;
...
// 	const axiosRetry$1 = requireCjs().default;
...
// 	const axiosRetry = requireAxiosRetry();
...
// 	validators = { validateRequiredFields, validateEmail, sanitizeInput };
// 	return validators;
// }
...
// 	const { post } = requireHttp();
// 	const { generateJWE, generateJWS } = requireCrypto();
// 	const { validateRequiredFields, validateEmail, sanitizeInput } = requireValidators();
// 	const { logger } = requireLogger();
...
// 	 */
// 	async function initiateApiKeyPayment({ merchantTxnId, paymentData, merchantCallbackURL }, config) {
...
// 	      'paymentData.billingData': paymentData.billingData,
// 	      'paymentData.billingData.emailId': paymentData.billingData.emailId,
// 	    },
// 	    ['merchantTxnId', 'paymentData', 'merchantCallbackURL', 'paymentData.totalAmount', 'paymentData.txnCurrency', 'paymentData.billingData', 'paymentData.billingData.emailId']
// 	  );
...
// 	  } catch (error) {
// 	    logger.error(`API key payment failed: ${error.message}`);
...
// 	 * @param {Object} params - Payment parameters
// 	 * @param {string} params.merchantTxnId - Transaction ID
// 	 * @param {string} params.merchantUniqueId - Unique merchant ID
// 	 * @param {Object} params.paymentData - Payment details
...
// 	 * @returns {Promise<Object>} Payment response
// 	 */
// 	async function initiateJwtPayment({ merchantTxnId, merchantUniqueId, paymentData, captureTxn, merchantCallbackURL }, config) {
// 	  validateRequiredFields(
// 	    {
// 	      merchantTxnId,
// 	      merchantUniqueId,
...
// 	      'paymentData.billingData.emailId': paymentData.billingData.emailId,
// 	    },
// 	    ['merchantTxnId', 'merchantUniqueId', 'paymentData', 'captureTxn', 'merchantCallbackURL', 'paymentData.totalAmount', 'paymentData.txnCurrency', 'paymentData.billingData', 'paymentData.billingData.emailId']
// 	  );

// 	  paymentData.billingData.emailId = validateEmail(sanitizeInput(paymentData.billingData.emailId));
// 	  const payload = { merchantTxnId, paymentData, captureTxn, merchantCallbackURL };
...
// 	    logger.info(`JWT payment initiated: ${merchantTxnId}, Redirect: ${redirectUrl}, GID: ${gid}`);
...
// 	 * @returns {Promise<Object>} Payment response
// 	 */
// 	async function initiateSiPayment({ merchantTxnId, paymentData, standingInstruction, merchantCallbackURL }, config) {
// 	  validateRequiredFields(
// 	    {
// 	      merchantTxnId,
// 	      paymentData,
...
// 	  paymentData.billingData.emailId = paymentData.billingData?.emailId ? validateEmail(sanitizeInput(paymentData.billingData.emailId)) : undefined;
// 	  paymentData.cardData.number = sanitizeInput(paymentData.cardData.number);
...
// 	 * @returns {Promise<Object>} Payment response
// 	 */
// 	async function initiateAuthPayment({ merchantTxnId, paymentData, billingData, captureTxn, merchantCallbackURL }, config) {
// 	  validateRequiredFields(
// 	    {
// 	      merchantTxnId,
// 	      paymentData,
...
// 	      'paymentData.totalAmount': paymentData.totalAmount,
// 	      'paymentData.txnCurrency': paymentData.txnCurrency,
// 	      'billingData.addressStreet1': billingData.addressStreet1,
// 	      'billingData.addressCity': billingData.addressCity,
// 	      'billingData.addressState': billingData.addressState,
// 	      'billingData.addressStateCode': billingData.addressStateCode,
// 	      'billingData.addressPostalCode': billingData.addressPostalCode,
// 	      'billingData.addressCountry': billingData.addressCountry,
// 	      'billingData.emailId': billingData.emailId,
// 	    },
// 	    [
// 	      'merchantTxnId',
// 	      'paymentData',
...
// 	      'paymentData.totalAmount',
// 	      'paymentData.txnCurrency',
// 	      'billingData.addressStreet1',
// 	      'billingData.addressCity',
// 	      'billingData.addressState',
// 	      'billingData.addressStateCode',
// 	      'billingData.addressPostalCode',
// 	      'billingData.addressCountry',
// 	      'billingData.emailId',
// 	    ]
// 	  );

// 	  billingData.emailId = validateEmail(sanitizeInput(billingData.emailId));
// 	  const payload = { merchantTxnId, paymentData, billingData, captureTxn, merchantCallbackURL };
// 	  const jwe = await generateJWE(payload, config);
// 	  const jws = await generateJWS(jwe, config);
...
// 	payment = { initiateApiKeyPayment, initiateJwtPayment, initiateSiPayment, initiateAuthPayment };
// 	return payment;
// }

// var refund;
// var hasRequiredRefund;

// function requireRefund () {
// 	if (hasRequiredRefund) return refund;
// 	hasRequiredRefund = 1;
// 	const { post } = requireHttp();
// 	const { generateJWE, generateJWS } = requireCrypto();
// 	const { validateRequiredFields } = requireValidators();
// 	const { logger } = requireLogger();
...
// var capture;
// var hasRequiredCapture;

// function requireCapture () {
// 	if (hasRequiredCapture) return capture;
// 	hasRequiredCapture = 1;
// 	const { post } = requireHttp();
// 	const { generateJWE, generateJWS } = requireCrypto();
// 	const { validateRequiredFields } = requireValidators();
// 	const { logger } = requireLogger();
...
// 	 * @param {string} params.gid - Global transaction ID
// 	 * @param {string} params.merchantTxnId - Transaction ID
// 	 * @param {string} params.captureType - Capture type
// 	 * @param {Object} params.paymentData - Payment details
// 	 * @param {Object} config - Configuration object
...
// 	  validateRequiredFields(
// 	    { gid, merchantTxnId, captureType, 'paymentData.totalAmount': paymentData.totalAmount },
// 	    ['gid', 'merchantTxnId', 'captureType', 'paymentData.totalAmount']
// 	  );

// 	  const payload = { merchantTxnId, captureType, paymentData };
// 	  const jwe = await generateJWE(payload, config);
// 	  const jws = await generateJWS(jwe, config);

// 	  try {
// 	    logger.info(`Initiating capture: ${gid}`);
// 	    const response = await post(
...
// 	      status: response.data.status || 'INITIATED',
// 	      gid: response.data.gid || gid,
// 	      message: response.data.message || 'Capture request processed',
...
// 	capture = { initiateCapture };
// 	return capture;
// }

// var reversal;
// var hasRequiredReversal;

// function requireReversal () {
// 	if (hasRequiredReversal) return reversal;
// 	hasRequiredReversal = 1;
// 	const { post } = requireHttp();
// 	const { generateJWE, generateJWS } = requireCrypto();
// 	const { validateRequiredFields } = requireValidators();
// 	const { logger } = requireLogger();
...
// 	hasRequiredStatus = 1;
// 	const { get } = requireHttp();
// 	const { pemToKey, generateJWS } = requireCrypto();
// 	const crypto = require$$1;
// 	const { logger } = requireLogger();
...
// 	 */
// 	async function checkStatus(gid, config) {
// 	  if (!gid) {
...
// 	requireCrypto();
// 	const { initiateApiKeyPayment, initiateJwtPayment, initiateSiPayment, initiateAuthPayment } = requirePayment();
// 	const { initiateRefund } = requireRefund();
// 	const { initiateCapture } = requireCapture();
// 	const { initiateAuthReversal } = requireReversal();
// 	const { checkStatus } = requireStatus();
...
// 	  async checkStatus(gid) {
// 	    return checkStatus(gid, this.config);
// 	  }
// 	}
...
// 	// const { generateJWE, generateJWS, pemToKey } = require('./core/crypto');
...
// 	//     return generateJWE(payload, this.config);
// 	//   }

// 	//   async #generateJWS(jwe) {
// 	//     return generateJWS(jwe, this.config);
// 	//   }
...
// 	//   async initiateApiKeyPayment({ merchantTxnId, paymentData, merchantCallbackURL }) {
...
// 	//     try {
// 	//       const response = await post(
// 	//         `${this.config.baseUrl}/gl/v1/payments/initiate/paycollect`,
// 	//         payload,
// 	//         {
...
// 	//   // Add other methods (initiateJwtPayment, initiateSiPayment, etc.) similarly
// 	// }
