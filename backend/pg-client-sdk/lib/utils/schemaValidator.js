 ...
 * Warns if objects are misplaced in the structure
...
function validateHierarchicalPlacement(payload) {
  ...
  // Define expected object hierarchy
const expectedHierarchy = {
  ...
  return {
    isValid: true,
    warnings,
    misplacedObjects: warnings
  };
}

/**
 * Validate payload - checks required fields and hierarchical placement
...
function validatePayload(payload, options = {}) {
  const {
    operation = 'payment',
    requiredFields = ['merchantTxnId', 'merchantCallbackURL', 'paymentData'],
  ...
  if (missingFields.length > 0) {
    ...
    logger.error('Validation failed - missing required fields', { missingFields, operation });
    throw new Error(JSON.stringify(errorResponse));
  }

  // 2. Check hierarchical placement (warnings only)
  ...
  logger.debug(`Payload validation passed for ${operation}`);
  
  return { 
    message: `Payload is valid for ${operation}`,
    warnings: hierarchicalValidation.warnings,
    warningCount: hierarchicalValidation.warnings.length
  };
}
