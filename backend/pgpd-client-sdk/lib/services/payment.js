...
const { validatePayDirectPayload } = require('../utils/schemaValidator');
...
async function initiateApiKeyPayment(payload, config) {
 ...
 try {
    validatePayDirectPayload(payload); 
  } catch (err) {
...
validateRequiredFields(
  ...
  if ('captureTxn' in payload && captureTxn !== true) {
    throw new Error(
      'Invalid value for captureTxn: This field is not required in JWT payments. If present, it must be set to true.'
    );
  }
  // if (paymentData?.billingData?.emailId) {
  //   paymentData.billingData.emailId = validateEmail(sanitizeInput(paymentData.billingData.emailId));
  ...
  try {
    ...
    const redirectUrl = response.data.redirectUrl || response.data.redirect_url || response.data.payment_link;
    const statusUrl = response.data.statusUrl || response.data.status_url || null;
    ...
    const gid = statusUrl ? statusUrl.match(/\/payments\/([^/]+)\/status/)?.[1] : null;
    // console.info(`JWT payment initiated: ${merchantTxnId}, Redirect: ${redirectUrl}, GID: ${gid}`);
    return { paymentLink: redirectUrl, gid };
  } catch (error) {
...
validateRequiredFields(
  ...
  // Clean SI fields
  const siType = standingInstruction?.data?.type?.trim();
  const siFrequency = standingInstruction?.data?.frequency?.trim();
  const siStartDate = standingInstruction?.data?.startDate;
  const amount = standingInstruction?.data?.amount?.trim();
  const maxAmount = standingInstruction?.data?.maxAmount?.trim();

  if (siType === 'FIXED' && !siStartDate) {
    throw new Error('startDate is required for FIXED SI type');
  }

  if (siType === 'VARIABLE' && siStartDate) {
  ...
  try {
    ...
    const gid = statusUrl ? statusUrl.match(/\/payments\/([^/]+)\/status/)?.[1] : null;
    logger.info(`SI payment initiated: ${merchantTxnId}, Redirect: ${redirectUrl}, GID: ${gid}`);
    return { paymentLink: redirectUrl, gid };
  } catch (error) {
...
if (captureTxn === undefined || captureTxn === null) {
  throw new Error('captureTxn is required in payload for Auth payment and must be explicitly set to false.');
}

if (captureTxn !== false) {
  throw new Error('Invalid value for captureTxn: For Auth payments, it must be set to false.');
}
  ...
  try {
    ...
    const gid = statusUrl ? statusUrl.match(/\/payments\/([^/]+)\/status/)?.[1] : null;
    // logger.info(`Auth payment initiated: ${merchantTxnId}, Redirect: ${redirectUrl}, GID: ${gid}`);
    return { paymentLink: redirectUrl, gid };
  } catch (error) {
