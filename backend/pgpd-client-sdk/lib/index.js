...
const { pemToKey, generateJWE, generateJWS } = require('./core/crypto');
...
const { initiatePauseSI } = require('./services/siPause');
const { initiateActivateSI} = require('./services/siActivate');
 ...
 */
class PayPdGlocalClient {
  constructor(config = {}) {
    this.config = new Config(config);
  }

  async initiateApiKeyPayment(params) {
    console.log('🔁 Initiating API Key Payment with params:', params);
  ...
  async initiateCheckStatus(gid) {
    return initiatecheckStatus(gid, this.config);
  }
...
module.exports = PayPdGlocalClient;
