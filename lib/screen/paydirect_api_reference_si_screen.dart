...
class PayDirectSiAPIReferenceScreen extends StatefulWidget {
  const PayDirectSiAPIReferenceScreen({super.key});

  @override
  State<PayDirectSiAPIReferenceScreen> createState() =>
      _PayDirectSiAPIReferenceScreenState();
}

class _PayDirectSiAPIReferenceScreenState
    extends State<PayDirectSiAPIReferenceScreen> {
        ...
        leading: IconButton(
          ...
          onPressed: () {
            ...
            if (AppNavigation.canPop()) {
              ...
              AppNavigation.to('/paydirect-si-detail');
            }
          },
  ...
  Widget _buildFixedSiSection(bool isDark) {
    return _build4StepApiSection(
      ...
      description: 'Create a recurring payment mandate with a fixed amount per billing cycle using card data. Ideal for subscriptions, loan EMIs, and regular payments.',
      method: 'POST',
  ...
  Widget _buildVariableSiSection(bool isDark) {
    return _build4StepApiSection(
      ...
      description: 'Create a recurring payment mandate with variable amounts up to a maximum limit using card data. Perfect for utility bills, usage-based services, and on-demand charges.',
      method: 'POST',
      endpoint: '/gl/v1/payments/initiate',
      payloadJson: '''{
...
var payloadJson = @"$csharpJson";''';
  ...
  String _getRequestExample(String apiId) {
    final examples = {
      'si-setup': {
        'curl': '''curl -X POST 'https://api.uat.payglocal.in/gl/v1/payments/initiate' \\
  -H 'Content-Type: text/plain' \\
